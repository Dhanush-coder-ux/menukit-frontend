...
import { Upload, FileText, ImageIcon, Check, Sparkles, Loader2, ChevronLeft } from 'lucide-react';
...
export function BulkUploadPage() {
  ...
  const processFile = async () => {
    ...
    try {
      const res = await api.post('/bulk-upload/parse', formData, {
      ...
      setStep('preview');
      toast.success(`Extracted ${items.length} items successfully`);
    } catch (error: any) {
      toast.error(error.response?.data?.detail || 'Failed to parse the menu. Please try again.');
    } finally {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold font-heading flex items-center gap-2">
            AI Bulk Upload <Sparkles size={20} className="text-amber-500" />
          </h2>
          <p className="text-slate-500">Upload a PDF or Image of your menu to automatically extract items.</p>
        </div>
      </div>

      {step === 'upload' && (
        <Card className="border-dashed border-2 overflow-hidden bg-slate-50/50 dark:bg-slate-900/20">
          <CardContent className="p-8 sm:p-12">
            {!file ? (
              <div 
                className="flex flex-col items-center justify-center text-center cursor-pointer py-10"
              ...
              >
                ...
                />
                <div className="w-20 h-20 bg-primary-50 text-primary dark:bg-primary-900/30 dark:text-primary-400 rounded-full flex items-center justify-center mb-6 shadow-sm">
                  <Upload size={32} />
                </div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Drag & Drop your menu here</h3>
                <p className="text-slate-500 max-w-sm mx-auto mb-6">
                  Supports PDF, JPG, PNG and WEBP up to 5MB. Our AI will read the text and structure it for you.
                </p>
              ...
              <div className="flex flex-col items-center justify-center py-8">
                <div className="w-16 h-16 bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 rounded-2xl flex items-center justify-center mb-4 shadow-sm">
                  {file.type.includes('pdf') ? <FileText size={32} /> : <ImageIcon size={32} />}
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <Button onClick={processFile} disabled={isUploading} className="min-w-[160px]">
                    {isUploading ? (
                      <><Loader2 size={18} className="mr-2 animate-spin" /> Processing AI...</>
                    ) : (
                      <><Sparkles size={18} className="mr-2" /> Extract Menu Items</>
                    )}
                  </Button>
                </div>
                ...
                {isUploading && (
                  <p className="text-xs text-slate-500 mt-6 animate-pulse">
                    This usually takes 10-20 seconds depending on menu size...
                  </p>
                )}
              </div>
            )}
      ...
      {step === 'preview' && (
        <div className="space-y-4 animate-fade-in">
          <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm sticky top-0 z-10">
            <div>
              <p className="text-sm text-slate-500">Extracted {parsedItems.length} items from</p>
              <p className="font-medium text-slate-900 dark:text-white flex items-center gap-2">
                {file?.name} 
                <button onClick={() => {setStep('upload'); setFile(null);}} className="text-xs text-primary hover:underline">Change File</button>
              </p>
            </div>
